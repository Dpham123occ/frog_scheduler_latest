<template>
  ...
  export default {
    ...
    methods: {
      ...
      async updateRequest() {
        try {
          const response = await axios.put(`https://frog-scheduler-latest.onrender.com/api/requests/${this.requestId}`, this.requestData);
      ...
      async cancelRequest(){
        try {
          await axios.delete(`https://frog-scheduler-latest.onrender.com/api/requests/${this.requestId}`);
